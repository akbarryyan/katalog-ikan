...
class Settings {
  ...
  static async getAll() {
    try {
      const [rows] = await db.execute('SELECT * FROM settings ORDER BY created_at DESC');
      return rows;
    } catch (error) {
  ...
  static async getByKey(key) {
    try {
      const [rows] = await db.execute('SELECT * FROM settings WHERE setting_key = ?', [key]);
      return rows[0] || null;
    } catch (error) {
  ...
  static async set(key, value, description = '') {
    try {
      ...
      if (existing) {
        ...
        return { id: existing.id, setting_key: key, setting_value: value, description, updated: true };
      } else {
        // Create new setting
        const [result] = await db.execute(
        ...
        return { id: result.insertId, setting_key: key, setting_value: value, description, updated: false };
      }
    } catch (error) {
  ...
  static async updateMultiple(settingsObject) {
    try {
      const results = [];
